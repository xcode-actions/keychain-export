...
struct KeychainExport : AsyncParsableCommand {
	...
	func run() async throws {
		/* The certificate is needed to find the private key, so we cannot skip searching for it. */
...
//				return data as NSData
		...
		let decodedPrivateKeyString: String?
		if !skipPrivateKey {
...
//				return data as NSData
			...
			decodedPrivateKeyString = try String(data: decodedPrivateKeyData, encoding: .ascii) ?! SimpleError("Cannot decode decoded pkey as ascii String.")
		} else {
			decodedPrivateKeyString = nil
		}
		
		if let certificateString       {print(certificateString)}
		if let decodedPrivateKeyString {print(decodedPrivateKeyString)}
	}
